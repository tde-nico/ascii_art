...
@njit(fastmath=True)
def accellerate_conversion(image, width, height, color_coeff, step):
	array_of_values = []
...
class ArtConverter:
	def __init__(self, path, pixel_size=7, color_lvl=8):
		...
		self.rec_fps = 25
		self.record = False
		self.recorder = cv2.VideoWriter('record.mp4',
			cv2.VideoWriter_fourcc(*'mp4v'), self.rec_fps, self.RES)
	...
	def draw_converted_image(self):
		...
		array_of_values = accellerate_conversion( self.image,
			self.WIDTH, self.HEIGHT, self.color_coeff, self.pixel_size)
...
if __name__ == '__main__':
	app = ArtConverter('video/test.mp4')
	app.run()
